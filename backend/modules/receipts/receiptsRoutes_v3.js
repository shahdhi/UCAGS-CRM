...
router.post('/generate', isAdmin, async (req, res) => {
  try {
    ...
    // Add seal image below thank you message (aligned slightly right of center, bigger)
    yPos += 25;
    const sealPath = path.join(__dirname, '../../../seal.png');
    if (fs.existsSync(sealPath)) {
      try {
        const sealSize = 100; // Bigger seal
    ...
    console.log('Drawing rounded footer');
    
    // Top block - College name (light purple) with rounded top corners
    const collegeBoxY = doc.page.height - 80;

...
router.post('/generate', isAdmin, async (req, res) => {
  try {
    ...
    const doc = new PDFDocument({
      size: [420, 560], // Width: 420pt, Height: 560pt (custom size)
    ...
    // "RECEIPT" text on left - aligned vertically with logo center (BOLD)
    ...
    payments.forEach((payment, index) => {
      ...
      // Draw horizontal white grid line at bottom of row (if not last row)
      if (index < payments.length - 1) {
        doc.moveTo(col1X, yPos + rowHeight).lineTo(doc.page.width - 30, yPos + rowHeight).stroke();
      }
    ...
    // Add seal image after total (positioned more to the right, with more space)
    yPos += 40;
    ...
    if (fs.existsSync(sealPath)) {
      try {
        ...
        yPos += sealSize + 10; // Reduced space after seal
      } catch (err) {
    ...
    // Minimum space between content and footer (very reduced)
    const minSpaceBeforeFooter = 5;
    const footerHeight = 80;
    const dynamicFooterY = Math.max(yPos + minSpaceBeforeFooter, doc.page.height - footerHeight);
    
    // Thank you message just above the footer
    const thankYouY = dynamicFooterY - 20;
    doc.fontSize(11)
       .fillColor('black')
       .font('Helvetica')
